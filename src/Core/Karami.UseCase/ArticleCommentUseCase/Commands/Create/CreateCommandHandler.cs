...
public class CreateCommandHandler : ICommandHandler<CreateCommand, string>
{
    ...
    public async Task<string> HandleAsync(CreateCommand command, CancellationToken cancellationToken)
    {
        var newComment = new ArticleComment(
            _dotrisDateTime, Guid.NewGuid().ToString(), command.OwnerId, command.ArticleId, command.Comment
        );

        #region OutBox

        var events = newComment.GetEvents.ToEntityOfEvent(_dotrisDateTime, _serializer, 
